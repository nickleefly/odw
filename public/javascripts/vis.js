(function() {
  ...
  $(function() {
    var all_data, base_vis, body, color, data, data_key, draw_details, draw_movie_details, draw_movies, filter_genres, filter_number, filter_year, h, hide_details, key_h, key_pb, key_pl, key_pr, key_pt, key_w, middle_line, movie_body, pb, pl, pr, pre_filter, pt, r_scale, render_key, render_vis, show_details, sort_data, update, update_data, update_scales, vis, w, xAxis, x_scale, yAxis, y_scale, y_scale_reverse, zero_line, _ref, _ref2,
      _this = this;
    data_key = {
      year: "year",
      rating: "Rating"
    ...
    draw_movies = function() {
      ...
      movies.enter().append("g").attr("class", "movie").on("mouseover", function(d, i) {
        ...
        return r_scale(parseFloat(d["year"]));
      });
      movies.transition().duration(1000).attr("transform", function(d) {
        return "translate(" + (x_scale(d["year"])) + "," + (y_scale(d["Rating"])) + ")";
      });
    ...
    render_key = function() {
      ...
      return key_demo_group.append("text").attr("dx", example_x + (example_r * 2) + 4).attr("dy", example_y + example_r + 6).text("year");
    };
    render_vis = function(csv) {
      ...
      base_vis.append("text").attr("x", w / 2).attr("y", h + (pt + pb) - 10).attr("text-anchor", "middle").attr("class", "axisTitle").attr("transform", "translate(" + pl + ",0)").text("year");
      ...
      vis.append("text").attr("x", h / 2).attr("y", 20).attr("text-anchor", "middle").attr("class", "axisTitle").attr("transform", "rotate(270)scale(-1,1)translate(" + pb + "," + 0 + ")").text("Rating (Rating 1 to 5)");
    ...
    show_details = function(movie_data, index, element) {
      ...
      box.x = Math.round(x_scale(movie_data["year"])) - (pr + 109) + r_scale(movie_data["year"]);
      box.y = Math.round(y_scale_reverse(movie_data["Rating"])) - (r_scale(movie_data["year"]) + pt + -55);
      tooltipWidth = parseInt(d3.select('#tooltip').style('width').split('px').join(''));
      msg = '<p class="title">' + movie_data["Film"] + '</p>';
      msg += '<table>';
      msg += '<tr><td>Rating:</td><td>' + movie_data["Rating"] + '</td></tr>';
      msg += '<tr><td>Zipcode:</td><td>' + movie_data["zipcode"] + '</td></tr>';
      msg += '<tr><td>Movie Id:</td><td>' + movie_data["id"] + '</td></tr>';
      msg += '</table>';
      ...
      crosshairs_g.append("line").attr("class", "crosshair").attr("x1", 0 + 3).attr("x2", x_scale(movie_data["year"]) - r_scale(movie_data["year"])).attr("y1", y_scale(movie_data["Rating"])).attr("y2", y_scale(movie_data["Rating"])).attr("stroke-width", 1);
      return crosshairs_g.append("line").attr("class", "crosshair").attr("x1", x_scale(movie_data["year"])).attr("x2", x_scale(movie_data["year"])).attr("y1", 0 + 3).attr("y2", y_scale(movie_data["Rating"]) - r_scale(movie_data["year"])).attr("stroke-width", 1);
    };
